...
<div id="chatter" class="discussion">
	...
	<div id="chatter_header" style="background-color:{{ $discussion->color }}">
		<div class="container">
			...
			<h1>{{ $discussion->title }}</h1><span class="chatter_head_details">Objavljeno v {{ Config::get('chatter.titles.category') }}<a class="chatter_cat" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
		</div>
	</div>
	        ...
	        <div class="col-md-12">
		                		...
		                		<span class="chatter_posts">
		                			@if(!Auth::guest() && (Auth::user()->id == $post->user->id))
		                				<div id="delete_warning_{{ $post->id }}" class="chatter_warning_delete">
		                					...
		                					<button class="btn btn-sm btn-default pull-right">Ne</button>
		                				</div>
			                			<div class="chatter_post_actions">
			                				<p class="chatter_delete_btn">
			                					<i class="chatter-delete"></i> Izbriši
			                				</p>
			                				<p class="chatter_edit_btn">
			                					<i class="chatter-edit"></i> Uredi
			                				</p>
			                			</div>
			                		@endif
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
				            ...
				            <form id="chatter_form_editor" action="/{{ Config::get('chatter.routes.home') }}/posts" method="POST">
						    	...
						    	<div id="editor">
									@if( $chatter_editor == 'tinymce' || empty($chatter_editor) )
										<label id="tinymce_placeholder">Dodaj vsebino</label>
						...
						<div id="discussion_response_email">
							<button id="submit_response" class="btn btn-success pull-right"><i class="chatter-new"></i> Odgovori</button>
							@if(Config::get('chatter.email.enabled'))
								<div id="notify_email">
									<img src="/vendor/devdojo/chatter/assets/images/email.gif" class="chatter_email_loader">
									<!-- Rounded toggle switch -->
									<span>Obvesti me o novih objavah</span>
									<label class="switch">
				...
				@else
					...
					<div id="login_or_register">
						<p>Prosimo, da se <a href="/{{ Config::get('chatter.routes.home') }}/login">prijaviš </a> ali <a href="/{{ Config::get('chatter.routes.home') }}/register">regisriraš</a> preden lahko komentiraš.</p>
					</div>
